...
const labels: Record<AppLocale, Record<StoryCategory | "other", string>> = {
  "zh-CN": { main: "主线剧情", friendship: "羁绊剧情", "live-result": "演出结束会话", home: "主页地点剧情", tutorial: "教程剧情", other: "其它剧情" },
  "ja-JP": { main: "メインストーリー", friendship: "絆ストーリー", "live-result": "ライブ終了会話", home: "ホーム会話", tutorial: "チュートリアル", other: "その他ストーリー" },
...
export default function StoryExplorer({ locale, initialCategory }: { locale: AppLocale; initialCategory: StorySection }) {
  ...
  const filters = (disableCollapse: boolean) => <BaseFilters title={labels[locale][initialCategory]} searchValue={query} onSearchChange={setQuery} searchLabel="Search" searchPlaceholder={locale === "zh-CN" ? "标题、角色、章节或 ADV ID" : "Title, character, chapter or ADV ID"} resultCount={filtered.length} totalCount={categoryTotal} hasActiveFilters={Boolean(query || otherCategories.length)} onReset={reset} resetLabel={locale === "zh-CN" ? "重置筛选" : "Reset"} expandLabel={locale === "zh-CN" ? "展开" : "Expand"} disableCollapse={disableCollapse}>{initialCategory === "other" ? <FilterSection title={locale === "zh-CN" ? "剧情类型" : "Story type"}><div className="flex flex-wrap gap-2"><FilterButton active={otherCategories.length === 0} onClick={() => setOtherCategories([])}>ALL</FilterButton>{(["live-result", "home", "tutorial"] as StoryCategory[]).map((category) => <FilterButton key={category} active={otherCategories.includes(category)} onClick={() => setOtherCategories(otherCategories.includes(category) ? otherCategories.filter((item) => item !== category) : [...otherCategories, category])}>{labels[locale][category]}</FilterButton>)}</div></FilterSection> : <div />}</BaseFilters>;

  if (initialCategory === "main") {
    if (loading) return <State text={locale === "zh-CN" ? "正在整理主线章节…" : "Loading main story chapters…"} />;
    if (error) return <State text={locale === "zh-CN" ? "主线剧情数据加载失败" : "Failed to load main stories"} action={() => setReload((v) => v + 1)} />;
  ...
  return <>
    ...
    <div className="grid min-w-0 gap-6 lg:grid-cols-[18rem_minmax(0,1fr)] lg:items-start"><aside className="sticky top-24 hidden lg:block">{filters(true)}</aside><section className="min-w-0" aria-live="polite">
      {loading ? <State text={locale === "zh-CN" ? "正在整理剧情资料…" : "Loading stories…"} /> : error ? <State text={locale === "zh-CN" ? "剧情数据加载失败" : "Failed to load story data"} action={() => setReload((v) => v + 1)} /> : filtered.length === 0 ? <State text={locale === "zh-CN" ? "没有符合条件的剧情" : "No matching stories"} action={reset} /> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{filtered.map((story) => <StoryCard key={story.id} story={story} locale={locale} onOpen={() => setActive(story)} />)}</div>}
    </section></div>
    <div className="lg:hidden"><QuickFilterButton title={locale === "zh-CN" ? "剧情筛选" : "Story filters"} buttonLabel={locale === "zh-CN" ? "筛选" : "Filter"} content={filters(true)} /></div>
...
function StoryReader({ story, locale, characters, texts, onClose }: { story: StoryViewModel | null; locale: AppLocale; characters: RawStoryCharacter[]; texts: RawText[]; onClose: () => void }) {
  ...
  return <Modal isOpen={Boolean(story)} onClose={onClose} title={story?.title} closeLabel={locale === "zh-CN" ? "关闭" : "Close"} size="xl">
    <audio
      ref={audioRef}
      preload="none"
    ...
    {failed ? <State text={locale === "zh-CN" ? "脚本资源暂不可用" : "Script asset unavailable"} /> : !script ? <State text={locale === "zh-CN" ? "正在解析脚本与语音…" : "Parsing script and voices…"} /> : (
